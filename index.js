...
const commaFormatter = (nominal, settings) => {
  if (!nominal) {
    ...
    if (diffLength > 0) {
      ...
      nominal = nominal.slice(0, settings.floatingPoint);
    }
  }
  return nominal;
};
...
/* DENOMINATE: 1000 -> 1, 500 -> 0.5 */
const denominateRecursive = (raw, settings) => {
  let val = Number(raw);
  if (isNaN(val)) return raw;

  const units = settings.longUnit
    ? ["ribu", "juta", "milyar", "triliun"]
    : [settings.k ? "k" : "rb", "jt", "M", "T"]; // 10^3 .. 10^12

  let exp = 0;
  while (val >= 1000 && exp < units.length - 1) {
    val = val / 1000;
    exp++;
  }

  const rounded = round2(val);
  const unit = units[exp - 1];

  // combine results
  const str = rounded.toString().replace(".", settings.decimal);
  return settings.spaceBeforeUnit && unit ? str + " " + unit : str + unit;
};
...
const defaultSettings = {
  ...
  denominate: false, // denominasi rupiah: Rp100.000 -> Rp100
};
...
module.exports = function toRupiah(target, settings = {}) {
  ...
  if (settings.denominate) {
    return symbolFormatter(denominateRecursive(target, settings), settings);
  }
